...
[3] = LAYOUT(
    ...
    _______, 	LNK_BLE1,  	LNK_BLE2,  	LNK_BLE3,  	LNK_RF,   	_______,   	_______,   	_______,   	_______,   	_______,  	_______,   	_______,	_______, 	_______,	            _______,
    _______,	SNAP_TAP_TG,   	_______,   	_______,  	_______,   	_______,   	_______,   	_______,   	_______,   	_______,  	_______,   	DEV_RESET,	SLEEP_MODE, BAT_SHOW,	            _______,
    _______,	_______,   	_______,   	_______,  	_______,   	_______,   	_______,	_______,   	_______,   	_______,  	_______,	_______, 	_______,                            _______,
    _______,				_______,   	_______,   	_______,  	_______,   	_______,   	_______,	MO(4), 		RGB_SPD,	RGB_SPI,	_______,	_______,	            RGB_VAI,    _______,
...
snap_tap_mode_t snap_tap_mode = SNAP_TAP_CANCEL;
uint8_t current_snap_tap_mode = 1;

// Для exclusion: запоминаем, какая клавиша была отменена
static bool a_was_cancelled = false;
static bool d_was_cancelled = false;
static bool w_was_cancelled = false;
static bool s_was_cancelled = false;

bool process_record_user(uint16_t keycode, keyrecord_t *record) {
 // Переключение режима с автоматическим переключением RGB эффекта
    if (keycode == SNAP_TAP_TG && record->event.pressed) {
        snap_tap_mode = (snap_tap_mode + 1) % 3;

        // Обновляем переменную для RGB эффекта
        current_snap_tap_mode = snap_tap_mode;

        // Автоматически переключаем на RGB эффект snap_tap_effect
        if (snap_tap_mode != SNAP_TAP_OFF) {
            rgb_matrix_mode(RGB_MATRIX_CUSTOM_snap_tap_effect);
        } else {
            // Можно вернуться к предыдущему эффекту или стандартному
    ...
    switch (keycode) {
        case KC_A:
            if (record->event.pressed) {
                is_A_pressed = true;
                if (is_D_pressed) {
                    if (snap_tap_mode == SNAP_TAP_CANCEL) {
                        unregister_code(KC_D);
                    } else if (snap_tap_mode == SNAP_TAP_EXCLUDE) {
                        unregister_code(KC_D);
                        d_was_cancelled = true;
                    }
                }
            } else {
                is_A_pressed = false;
                if (snap_tap_mode == SNAP_TAP_EXCLUDE && d_was_cancelled && is_D_pressed) {
                    register_code(KC_D);
            ...
            if (record->event.pressed) {
                ...
                if (is_A_pressed) {
                    if (snap_tap_mode == SNAP_TAP_CANCEL) {
                        ...
                        unregister_code(KC_A);
                        a_was_cancelled = true;
                    }
                }
            } else {
                is_D_pressed = false;
                if (snap_tap_mode == SNAP_TAP_EXCLUDE && a_was_cancelled && is_A_pressed) {
                    register_code(KC_A);
                    a_was_cancelled = false;
                }
            }
            break;
        case KC_W:
            if (record->event.pressed) {
                ...
                if (is_S_pressed) {
                    if (snap_tap_mode == SNAP_TAP_CANCEL) {
                        ...
                        unregister_code(KC_S);
                        s_was_cancelled = true;
                    }
                }
            } else {
                is_W_pressed = false;
                if (snap_tap_mode == SNAP_TAP_EXCLUDE && s_was_cancelled && is_S_pressed) {
                    register_code(KC_S);
                    s_was_cancelled = false;
                }
            }
            break;
        case KC_S:
            if (record->event.pressed) {
                ...
                if (is_W_pressed) {
                    if (snap_tap_mode == SNAP_TAP_CANCEL) {
                        ...
                        unregister_code(KC_W);
                        w_was_cancelled = true;
                    }
                }
            } else {
                is_S_pressed = false;
                if (snap_tap_mode == SNAP_TAP_EXCLUDE && w_was_cancelled && is_W_pressed) {
                    register_code(KC_W);
                    w_was_cancelled = false;
                }
            }
            break;
    }
